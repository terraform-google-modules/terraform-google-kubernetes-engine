...
control "gcloud" do
  # TODO ensure this is actually a regional cluster
  ...
  describe command("gcloud --project=#{project_id} container clusters --zone=#{location} describe #{cluster_name} --format=json") do
    ...
    describe "cluster" do
      ...
      it "is in the expected network" do
        expect(metadata['network']).to eq network
      end

      it "is in the expected subnetwork" do
        expect(metadata['subnetwork']).to eq subnetwork
      end

      it "has the expected secondary ip range for pods" do
        expect(metadata['ipAllocationPolicy']['clusterSecondaryRangeName']).to eq ip_range_pods
      end

      it "has the expected secondary ip range for services" do
        expect(metadata['ipAllocationPolicy']['servicesSecondaryRangeName']).to eq ip_range_services
      end

      it "has the expected addon settings" do
        expect(metadata['addonsConfig']).to eq({
    ...
    describe "default node pool" do
      let(:default_node_pool) { metadata['nodePools'].select { |p| p['name'] == "default-pool" }.first }
    ...
    describe "node pool" do
      let(:node_pool) { metadata['nodePools'].reject { |p| p['name'] == "default-pool" }.first }
