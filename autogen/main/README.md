...
| pod_cidr_overprovision_config | Configuration for pod CIDR over-provisioning. Set as `{ disabled = true }` to disable GKE's default doubling of the per-node pod IP range allocation. |  | Optional |
