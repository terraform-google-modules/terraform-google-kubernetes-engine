...
if [[  "${CA_CERT}" != "none" ]]; then
    CA_CERT_COMMAND_SNIPPET="--ca_cert ${CA_CERT}"
else
    CA_CERT_COMMAND_SNIPPET=""
fi

if [[  "${CA_KEY}" != "none" ]]; then
    CA_KEY_COMMAND_SNIPPET="--ca_key ${CA_KEY}"
else
    CA_KEY_COMMAND_SNIPPET=""
fi

if [[  "${ROOT_CERT}" != "none" ]]; then
    ROOT_CERT_COMMAND_SNIPPET="--root_cert ${ROOT_CERT}"
else
    ROOT_CERT_COMMAND_SNIPPET=""
fi

if [[  "${CERT_CHAIN}" != "none" ]]; then
    CERT_CHAIN_COMMAND_SNIPPET="--cert-chain ${CERT_CHAIN}"
else
...
echo -e "install_asm_${ASM_VERSION} --verbose --project_id ${PROJECT_ID} --cluster_name ${CLUSTER_NAME} --cluster_location ${CLUSTER_LOCATION} --mode ${MODE} ${MCP_COMMAND_SNIPPET} ${OPTIONS_COMMAND_SNIPPET} ${CUSTOM_OVERLAYS_COMMAND_SNIPPET} ${OUTDIR_COMMAND_SNIPPET} ${ENABLE_ALL_COMMAND_SNIPPET} ${ENABLE_CLUSTER_ROLES_COMMAND_SNIPPET} ${ENABLE_CLUSTER_LABELS_COMMAND_SNIPPET} ${ENABLE_GCP_COMPONENTS_COMMAND_SNIPPET} ${ENABLE_REGISTRATION_COMMAND_SNIPPET} ${CA_COMMAND_SNIPPET} ${SERVICE_ACCOUNT_COMMAND_SNIPPET} ${KEY_FILE_COMMAND_SNIPPET} ${CA_CERT_COMMAND_SNIPPET} ${CA_KEY_COMMAND_SNIPPET} ${ROOT_CERT_COMMAND_SNIPPET} ${CERT_CHAIN_COMMAND_SNIPPET} ${REVISION_NAME_COMMAND_SNIPPET}"
