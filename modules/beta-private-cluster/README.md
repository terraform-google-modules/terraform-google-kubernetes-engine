...
| managed\_opentelemetry\_scope | (Optional, Beta) The scope of the GKE Managed OpenTelemetry pipeline. Accepted values are `SCOPE_UNSPECIFIED`, `NONE`, and `COLLECTION_AND_INSTRUMENTATION_COMPONENTS`. When `null`, the `managed_opentelemetry_config` block is omitted. | `string` | `null` | no |
