...
| subnetwork | The subnet for this node pool. Accepts either a bare subnet name or a full `projects/{project}/regions/{region}/subnetworks/{subnetwork}` path. Once set, the node pool's subnetwork is immutable. Note: this is a node-pool-level override, distinct from the cluster-level `subnetwork` module input. |  | Optional |
